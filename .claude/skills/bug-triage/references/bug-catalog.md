...
- 2026-09-15 · reservation never reaches `CONFIRMED`; `consume()` unreachable in production · `POST /reservations/{id}/confirm` (tech:3635) was never implemented in `dispatch.py`/`reservation_service.py`, but `consume()` (`reservation_service.py:79-90`) hard-requires `ReservationStatus.CONFIRMED` · none yet — add a test asserting HELD→CONFIRMED→IN_USE is reachable over HTTP before fixing (new shape: **terminal-transition endpoint missing while a downstream method requires that state** — not in Part A, add there too)
- 2026-09-15 · dispatcher's "Why selected?" decision trace shows the same numbers for every ambulance · `App.tsx:186-188` renders literal hardcoded strings (`"GPS fresh · 5 seconds old"`, `"ETA 7 min"`) instead of `selectedAmbulance.age_s`/`.eta_s`/`.reasons` · none yet — needs a component test asserting the panel text changes when a non-default candidate is selected (new shape: **explanation UI bound to a literal instead of the selected entity** — distinct from #18's simulation-labelling gap; this is fabricated content, not a missing label)
- 2026-09-15 · decision persistence and 4 more migrations (`0006` resource_events, `0010` route_alternatives, `0011` decision_trace_context, `0013`/`0014` simulation_scenarios/events) are unreadable/unwritable from the app · `decision_service.py::persist_decision` exists but is never called (grepped, zero call sites); the 5 tables/columns above have zero references anywhere under `apps/api/app/` · none yet — add an import-graph or coverage check that fails when a migration-created table has no ORM model or service reference (new shape: **schema shipped by migration, never wired into application code** — related to Part B's "config/code drift" but for DB schema, not config keys)
- 2026-09-15 · ambulance/hospital ranking scores don't actually discriminate on capability/acceptance/readiness · `decision_engine/ambulance.py:51` and `hospital.py:50` substitute literal constants (`1.0`, `0.5`) for `capability_score`/`acceptance_score`/`readiness_score` in the weighted sum, so those weight terms never vary by candidate; weights themselves are Python literals, not config-driven (tech:3184/3206 violated) · `tests/decision_engine/test_*_engine.py` cover eligibility/tie-break only, not weight correctness — add a test that varies capability/acceptance/readiness inputs and asserts the score moves
